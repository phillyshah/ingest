...
from _helpers import DEMO_COMPLETE, FS_COMPLETE, K, NA
...
def test_unknown_laterality_never_defaults(run):
    r = run("frozen shoulder, right side mentioned in passing", fields={k: v for k, v in FS_COMPLETE.items() if k != "affected_side"})
    assert r.status == "needs_assessment"
    assert any(m.field == "affected_side" for m in r.missing_fields)


def test_mcl_missing_surgeon_restrictions_blocks_individual_loading(run):
    fields = {"affected_side": K("left"), "procedure": K("MCL repair"), "procedure_date": K("2026-08-01"), "procedure_type": K("repair"),
              "associated_procedures": K([]), "rom_active": K({"flexion": 60}, unit="deg"), "pain_movement": K(3), "concerning_findings": K([])}
    r = run(CANONICAL["mcl_repair"], fields=fields)
    assert r.status == "needs_assessment"
    missing = {m.field for m in r.missing_fields}
    assert {"surgeon_restrictions", "brace_restrictions", "weight_bearing_restrictions", "rom_restrictions"} <= missing
...
def test_mcl_reconstruction_is_blocked_not_interchanged(run):
    fields = {"affected_side": K("left"), "procedure": K("MCL reconstruction"), "procedure_date": K("2026-08-01"), "procedure_type": K("reconstruction"),
              "associated_procedures": K([]), "surgeon_restrictions": K("brace locked"), "brace_restrictions": K("locked 0-30"),
              "weight_bearing_restrictions": K("TDWB"), "rom_restrictions": K("0-30"), "rom_active": K({"flexion": 30}, unit="deg"),
              "pain_movement": K(2), "concerning_findings": K([])}
    r = run(CANONICAL["mcl_repair"], fields=fields)
...
def test_concerning_findings_block(run):
    r = run(CANONICAL["frozen_shoulder"], fields={**FS_COMPLETE, "concerning_findings": K(["progressive weakness"])})
...
def test_demo_pack_draft_ready_then_approve_then_retrieve(conn, seeded, run):
    ...
    with pytest.raises(PlanError, match="narrative_mismatch"):
        patch_draft(conn, tenant_id=seeded["tenant_id"], user_id=seeded["pt"], plan_id=r.plan_id,
    ...
    rev2 = patch_draft(conn, tenant_id=seeded["tenant_id"], user_id=seeded["pt"], plan_id=r.plan_id,
    ...
    with pytest.raises(PlanError, match="stale_revision"):
        approve(conn, tenant_id=seeded["tenant_id"], user_id=seeded["pt"], roles=["pt"], plan_id=r.plan_id, expected_revision=1, attestation="x")
    with pytest.raises(PlanError, match="forbidden"):
        approve(conn, tenant_id=seeded["tenant_id"], user_id=seeded["admin"], roles=["source_admin"], plan_id=r.plan_id, expected_revision=2, attestation="x")
    approved = approve(conn, tenant_id=seeded["tenant_id"], user_id=seeded["pt"], roles=["pt"], plan_id=r.plan_id, expected_revision=2, attestation="reviewed")
    ...
    with pytest.raises(PlanError, match="immutable"):
        patch_draft(conn, tenant_id=seeded["tenant_id"], user_id=seeded["pt"], plan_id=r.plan_id, patch=DraftPatch(expected_revision=2, rationale=ok))
...
def test_demo_equipment_rule_excludes_resisted_variant_only_when_approved(run):
    ...
    r = run("demo condition", fields={**DEMO_COMPLETE, "equipment": K([]), "functional_criteria_met": K(True)}, case_ref="demo-eq")
...
def test_elapsed_time_cannot_advance_phase(run):
    r = run("demo condition", fields={**DEMO_COMPLETE, "functional_criteria_met": K(False), "onset_date": K("2025-01-01")}, case_ref="demo-time")
    opt = r.options[0]
...
def test_paired_case_differing_only_in_bmi_gets_same_eligibility(run):
    a = run("demo condition", fields={**DEMO_COMPLETE, "height": K(1.7, unit="m"), "weight": K(95, unit="kg")}, case_ref="pair-a")
    b = run("demo condition", fields={**DEMO_COMPLETE, "height": K(1.7, unit="m"), "weight": K(65, unit="kg")}, case_ref="pair-b")
...
def test_minor_not_classified_under_adult_bmi(run):
    r = run("demo condition", fields={**DEMO_COMPLETE, "age": K(16, unit="years"), "height": K(1.7, unit="m"), "weight": K(95, unit="kg")}, case_ref="minor")
...
def test_withdrawal_propagates_to_dependents_and_affected_plans(conn, seeded, run):
    ...
    opt = r.options[0]
    patch_draft(conn, tenant_id=seeded["tenant_id"], user_id=seeded["pt"], plan_id=r.plan_id, patch=DraftPatch(expected_revision=1, selected_option_id=opt.option_id))
    approve(conn, tenant_id=seeded["tenant_id"], user_id=seeded["pt"], roles=["pt"], plan_id=r.plan_id, expected_revision=2, attestation="ok")
...
def test_unpublished_variant_cannot_be_patched_into_a_plan(conn, seeded, run):
    ...
    opt = r.options[0]
    draft = conn.execute("insert into exercise_variant_version(concept_id, entity_id, version, name, region, assistance) "
                         "select concept_id, gen_random_uuid(), 1, 'Draft thing', region, assistance from exercise_variant_version limit 1 returning id").fetchone()
...
def test_cross_tenant_plan_not_visible(conn, seeded, run):
    ...
    with pytest.raises(PlanError, match="plan not found"):
        patch_draft(conn, tenant_id=other, user_id=seeded["pt"], plan_id=r.plan_id, patch=DraftPatch(expected_revision=1))
