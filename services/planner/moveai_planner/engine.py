...
from moveai_contracts.enums import DoseProvenance, FieldStatus, PlanStatus
...
def evaluate_rules(protocols: list[LoadedProtocol], intake: Intake) -> dict[str, Any]:
            ...
            if r.severity == "block" and act.type == "block_for_review":
                if ev.result is Tri.TRUE:
                    ...
                    for f in ev.unknown_fields:
                        missing.setdefault(f, MissingField(field=f, reason=f"needed to evaluate safety rule {r.name!r}; unknown is not a negative finding", required_by=r.name))
                continue
            if act.type == "exclude_variant":
                if ev.result is Tri.TRUE:
                    if lr.executable:
                        ...
                        unsigned_slots.append(f"{r.name}: would exclude {act.variant_keys or act.tags or act.assistance} but the rule is {lr.approval_state}; not applied")
                continue
    ...
    return {"missing": list(missing.values()), "blocks": blocks, "exclusions": exclusions, "informs": informs,
            "rank_adjust": rank_adjust, "unsigned_slots": unsigned_slots}
...
def _window(w: dict[str, Any]) -> TimeWindow:
    return TimeWindow(anchor=w.get("anchor", "assessment"), start=w.get("start"), end=w.get("end"), unit=w.get("unit", "weeks"),
                      provisional=bool(w.get("provisional", True)), text=w.get("text"), provenance=w.get("provenance", "unknown"))
...
def _source_refs(u: LoadedUse) -> list[SourceReference]:
    ...
    if sr:
        refs.append(SourceReference(url=sr.get("url"), publisher=sr.get("publisher"), title=sr.get("title"), document_version=sr.get("document_version"),
                                    locator=sr.get("locator"), source_exercise_name=sr.get("source_exercise_name")))
    for c in u.claims:
        refs.append(SourceReference(url=c["canonical_url"], publisher=c["publisher"], title=c["source_title"], document_version=c["document_identity"],
...
def _media_for(u: LoadedUse) -> tuple[str | None, str]:
    ...
    for m in u.media:
        if m["media_state"] == "graphic_available" and m.get("can_display_to_patient") == "allowed" and m.get("technique_review_passed") \
           and not m.get("revoked_at") and not (m.get("expires_at") and m["expires_at"] <= datetime.now(timezone.utc)):
...
def _item(u: LoadedUse, *, prescribed: bool) -> PlanItem:
    ...
    return PlanItem(
        variant_version_id=str(u.variant["id"]), variant_name=u.variant["name"], clinical_use_version_id=u.id,
        media_version_id=media_id, media_state=media_state,
        instructions=[s["text"] for s in (u.variant["step_sequence"] or [])],
        prescribed_dose=dose, dose_label=("prescribed" if prescribed and has_numbers else "protocol_example" if has_numbers else "none"),
        goal_ids=list(u.row["goals"] or []), evidence_claim_ids=[str(c) for c in u.row["supporting_claim_ids"]],
...
def _timeline(p: LoadedProtocol, intake: Intake, *, prescribed_phase: str | None) -> list[TimelineRow]:
    ...
    for ph in p.phases:
        ...
        rows.append(TimelineRow(
            phase_key=ph["key"], phase_name=ph["name"], goals=ph.get("goals") or [], time_window=_window(ph.get("window") or {}),
            exercises=items, schedule_note=("approved doses" if any(i.dose_label == "prescribed" for i in items)
                                            else "protocol example values" if any(i.dose_label == "protocol_example" for i in items)
                                            else "dose not supplied by source or clinician; PT must author"),
            restrictions=restrictions, unknown_restrictions=not restrictions_known,
            review_checkpoint=ph.get("review_checkpoint"), advance_criteria=ph.get("advance_criteria") or [],
            hold_criteria=ph.get("hold_criteria") or [], regress_criteria=ph.get("regress_criteria") or [],
...
def _preview(p: LoadedProtocol, intake: Intake) -> PathwayPreview:
    ...
    assumptions = ["Timing shown is relative and provisional; a scheduled date is a review opportunity, not proof of readiness.",
    ...
    return PathwayPreview(protocol_version_id=p.id, protocol_name=p.row["name"], condition=p.condition["name"], label=PREVIEW_LABEL,
                          approval_state=p.row["approval_state"], branch_conditions=prov.get("branch_conditions") or [],
                          timeline=_timeline(p, intake, prescribed_phase=None),
                          recovery_horizon=(p.row.get("recovery_horizon") or {}).get("text"), assumptions=assumptions,
...
def _hard_exclusions(p: LoadedProtocol, u: LoadedUse, intake: Intake, exclusions: list[tuple[dict, str, str]]) -> ExcludedCandidate | None:
    ...
    if not u.prescribable:
        return ExcludedCandidate(variant_version_id=str(v["id"]), protocol_version_id=p.id, reason=f"clinical use/variant is {u.row['approval_state']}/{v['approval_state']}; unpublished versions cannot enter a plan")
    mods = v.get("supported_modifications") or {}
    tags = set(mods.get("tags", []))
    for act, rid, name in exclusions:
        if str(v["id"]) in act.get("variant_keys", []) or v["name"] in act.get("variant_keys", []) or mods.get("pack_key") in act.get("variant_keys", []) or \
           any(k and k in (v.get("source_reference") or {}).get("source_exercise_name", "") for k in act.get("variant_keys", [])) or \
           v["assistance"] in act.get("assistance", []) or tags & set(act.get("tags", [])):
            return ExcludedCandidate(variant_version_id=str(v["id"]), protocol_version_id=p.id, reason=f"excluded by approved rule {name!r}", rule_version_id=rid)
    ...
    if equipment.is_known and v["equipment"]:
        ...
        if not need <= have:
            return ExcludedCandidate(variant_version_id=str(v["id"]), protocol_version_id=p.id, reason=f"requires equipment not available: {sorted(need - have)}")
    ...
    if pos.is_known and pos.value is False and (v.get("supported_modifications") or {}).get("requires_starting_position"):
        return ExcludedCandidate(variant_version_id=str(v["id"]), protocol_version_id=p.id, reason="patient cannot assume the required starting position (observed limitation)")
    return None
...
def _rank(p: LoadedProtocol, u: LoadedUse, intake: Intake) -> dict[str, float]:
    ...
    if "home" in (u.variant["setting"] or []):
        practical += 0.5
    total = WEIGHTS["presentation"] * presentation + WEIGHTS["goal"] * goal + WEIGHTS["evidence"] * evidence + WEIGHTS["practical"] * practical
    return {"presentation_fit": presentation, "goal_fit": goal, "evidence_relevance": evidence, "practical_fit": practical, "score": round(total, 4)}
...
def validate_option(opt: PlanOption, intake: Intake) -> ValidationResult:
    ...
    for it in opt.items:
        ...
        est = 0.0
        sets = d.fields.get("sets"); reps = d.fields.get("repetitions"); hold = d.fields.get("hold_seconds")
...
def plan_options(conn: psycopg.Connection, *, tenant_id: Any, user_id: Any, submission: CaseSubmission) -> PlanOptionsResponse:
    ...
    prev = conn.execute("select coalesce(max(revision),0) as r from case_snapshot where tenant_id=%s and case_ref=%s", (tenant_id, submission.case_ref)).fetchone()["r"]
    snap = conn.execute(
        ...
        (tenant_id, submission.case_ref, prev + 1, submission.assessment_time, user_id, submission.narrative,
         J({k: v.model_dump(mode="json") for k, v in intake.fields.items()}),
         J({"known": [k for k, v in intake.fields.items() if v.is_known]}))).fetchone()
    observations = derive_observations(conn, tenant_id, snap["id"], intake)
    bmi_def = ensure_bmi_definition(conn)

    resp = PlanOptionsResponse(status=PlanStatus.needs_assessment, case_snapshot_id=str(snap["id"]), case_revision=snap["revision"],
                               candidate_conditions=[c["preferred_name"] for c in candidates], assumptions=notes,
                               catalog_release_id=str(release["id"]) if release else None)
    if not candidates:
        resp.missing_fields = [MissingField(field="diagnosis", reason="No candidate condition recognized; state the confirmed or suspected diagnosis and assessor")]
    ...
    protocols = load_protocols(conn, [c["id"] for c in candidates], release_id=release["id"] if release else None, include_unsigned=True)
    applicability = [a for p in protocols for a in p.applicability] + [a for p in protocols for u in p.uses.values() for a in u.applicability]
    ...
    resp.timeline_anchor = ", ".join(sorted(f"{a} ({'known' if intake.known(a + '_date') or a == 'assessment' else 'date unknown'})" for a in anchors)) or None
    ...
    if not protocols:
        ...
        resp.blocked = BlockedInfo(rule_version_id=None, rule_name="no_pathway", message="No pathway (approved or placeholder) exists for the recognized condition.")
        return _persist(conn, tenant_id, user_id, snap, resp, intake)
    ...
    if not eligible:
        resp.status = PlanStatus.blocked_for_clinical_review
        resp.blocked = BlockedInfo(rule_version_id=None, rule_name="no_approved_pathway",
    ...
    for p in eligible[:MAX_OPTIONS]:
        ...
        if phase is None:
            excluded.append(ExcludedCandidate(protocol_version_id=p.id, reason="no phase entry criteria are met on the current assessment"))
            continue
        items: list[PlanItem] = []
        ...
        for uid in ph["items"]:
            ...
            for adj, name in rules["rank_adjust"]:
                r["score"] = round(r["score"] + adj, 4); r[f"adjust:{name}"] = adj
            items.append(it); ranks.append(r)
        if not items:
        ...
        items = [items[i] for i in order]; ranks = [ranks[i] for i in order]
        opt = PlanOption(option_id=f"opt-{uuid.uuid4().hex[:8]}", label=p.row["name"], case_snapshot_id=str(snap["id"]),
                         catalog_release_id=str(release["id"]) if release else None, protocol_version_ids=[p.id],
                         rule_version_ids=sorted(p.rules), items=items, timeline=_timeline(p, intake, prescribed_phase=phase),
    ...
    if not options:
        resp.status = PlanStatus.blocked_for_clinical_review
        resp.blocked = BlockedInfo(rule_version_id=None, rule_name="no_valid_items", message="No approved variant survived hard exclusions; PT authoring required.")
        return _persist(conn, tenant_id, user_id, snap, resp, intake)
...
def _persist(conn: psycopg.Connection, tenant_id: Any, user_id: Any, snap: dict, resp: PlanOptionsResponse, intake: Intake) -> PlanOptionsResponse:
    plan_id = uuid.uuid4()
    ...
    row = conn.execute(
        ...
        (tenant_id, plan_id, snap["id"], resp.catalog_release_id, resp.status.value, J(payload["options"]),
         J({"status": resp.status.value, "missing": payload["missing_fields"], "blocked": payload["blocked"]}),
         [uuid.UUID(x) for o in resp.options for x in o.protocol_version_ids], [uuid.UUID(x) for o in resp.options for x in o.rule_version_ids],
         content_hash(payload["options"]), user_id, J(payload["segment_explanations"]))).fetchone()
