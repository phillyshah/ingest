...
def latest_revision(conn: psycopg.Connection, tenant_id: Any, plan_id: Any) -> dict | None:
    return conn.execute("select * from plan_version where tenant_id=%s and plan_id=%s order by revision desc limit 1", (tenant_id, plan_id)).fetchone()
...
def patch_draft(conn: psycopg.Connection, *, tenant_id: Any, user_id: Any, plan_id: Any, patch: DraftPatch) -> dict:
    ...
    if patch.items is not None:
        ...
        for it in patch.items:
            ...
            if cur["catalog_release_id"] and not release_contains(conn, cur["catalog_release_id"], it.variant_version_id):
                raise PlanError("not_in_release", f"variant {it.variant_version_id} is not in the pinned catalog release", 422)
    ...
    row = conn.execute(
        ...
        (tenant_id, plan_id, cur["revision"] + 1, cur["case_snapshot_id"], cur["catalog_release_id"], cur["routing"], opt.option_id, J(payload),
         J([i.model_dump(mode="json") for i in opt.items]), J(opt.validation.model_dump()), patch.rationale or narrative_for(opt),
         [uuid.UUID(x) for x in opt.protocol_version_ids], [uuid.UUID(x) for x in opt.rule_version_ids], opt.content_hash, user_id,
         cur["segment_explanations"] if isinstance(cur["segment_explanations"], str) else J(cur["segment_explanations"]))).fetchone()
    conn.execute("update plan_version set status='superseded' where id=%s", (cur["id"],))
    conn.execute("insert into review_event(tenant_id, entity_table, version_id, reviewer_id, reviewer_role, decision, reason, changes) values (%s,'plan_version',%s,%s,'pt','edit',%s,%s)",
                 (tenant_id, row["id"], user_id, patch.edit_reason, J({"selected_option_id": opt.option_id, "items_changed": patch.items is not None})))
    return row
...
def _dependency_checks(conn: psycopg.Connection, row: dict, opt: PlanOption) -> list[str]:
    problems = []
    for it in opt.items:
        for table, vid in (("exercise_variant_version", it.variant_version_id), ("clinical_use_version", it.clinical_use_version_id)):
        ...
        if it.media_version_id:
            m = conn.execute("select m.media_state, g.can_display_to_patient, g.expires_at, g.revoked_at from media_asset_version m left join rights_grant g on g.id=m.rights_grant_id where m.id=%s", (it.media_version_id,)).fetchone()
        ...
        for cid in it.evidence_claim_ids:
            sv = conn.execute("select sv.pipeline_state, g.revoked_at, g.expires_at from evidence_claim c join source_version sv on sv.id=c.source_version_id "
                              "left join rights_grant g on g.source_version_id=sv.id where c.id=%s", (cid,)).fetchone()
...
def approve(conn: psycopg.Connection, *, tenant_id: Any, user_id: Any, roles: list[str], plan_id: Any, expected_revision: int, attestation: str) -> dict:
    cur = latest_revision(conn, tenant_id, plan_id)
    if not cur:
        raise PlanError("not_found", "plan not found", 404)
    ...
    newer = conn.execute("select 1 from plan_version where tenant_id=%s and plan_id=%s and revision > %s", (tenant_id, plan_id, cur["revision"])).fetchone()
    if newer:
        raise PlanError("stale_revision", "a newer revision exists")
    row = conn.execute("update plan_version set status='approved', approved_by=%s, approved_at=now(), approval_signature=%s where id=%s returning *",
                       (user_id, sig, cur["id"])).fetchone()
    conn.execute("insert into review_event(tenant_id, entity_table, version_id, reviewer_id, reviewer_role, decision, reason) values (%s,'plan_version',%s,%s,'pt','approve',%s)",
                 (tenant_id, cur["id"], user_id, attestation))
    conn.execute("insert into outbox_event(event_type, tenant_id, object_table, object_id, object_version, payload) values ('plan.approved',%s,'plan_version',%s,%s,%s)",
                 (tenant_id, cur["id"], cur["revision"], J({"plan_id": str(plan_id), "revision": cur["revision"], "content_hash": cur["content_hash"]})))
    return row
...
def get_approved(conn: psycopg.Connection, *, tenant_id: Any, plan_id: Any) -> dict:
    ...
    row = conn.execute("select * from plan_version where tenant_id=%s and plan_id=%s and status in ('approved','review_required') order by revision desc limit 1", (tenant_id, plan_id)).fetchone()
    if not row:
        latest = latest_revision(conn, tenant_id, plan_id)
        return {"available": False, "reason": (f"latest revision {latest['revision']} is {latest['status']} ({latest['routing']})" if latest else "unknown plan")}
    ...
    return {"available": True, "assignable": not review, "plan_id": str(plan_id), "revision": row["revision"], "case_snapshot_id": str(row["case_snapshot_id"]),
            "catalog_release_id": str(row["catalog_release_id"]) if row["catalog_release_id"] else None, "content_hash": row["content_hash"],
            "approval_signature": row["approval_signature"], "approved_at": row["approved_at"], "items": row["selected_items"],
            "option": next(o for o in row["options"] if o["option_id"] == row["option_id"]), "rationale": row["rationale"],
            "hold": review, "hold_reason": row["review_required_reason"] if review else None}
...
def propagate_withdrawal(conn: psycopg.Connection, *, entity_table: str, version_id: Any, reason: str) -> dict[str, Any]:
    ...
    while frontier:
        t, vid = frontier.pop()
        for e in conn.execute("select downstream_table, downstream_id from dependency_edge where upstream_table=%s and upstream_id=%s", (t, vid)).fetchall():
    ...
    for t, vid in affected_versions:
        if t in ("exercise_variant_version", "clinical_use_version", "protocol_version", "media_asset_version"):
            conn.execute(f"update {t} set approval_state='withdrawn', withdrawn_at=now(), withdrawal_reason=%s where id=%s and approval_state in ('approved','published','pending_review','draft')",
                         (f"upstream {entity_table} {version_id}: {reason}", vid))
            conn.execute("insert into catalog_change(change_type, entity_table, version_id) values ('withdrawn',%s,%s)", (t, vid))
            conn.execute("insert into outbox_event(event_type, object_table, object_id, payload) values ('content.withdrawn',%s,%s,%s)", (t, vid, J({"reason": reason})))
    ids = [vid for t, vid in affected_versions if t in ("exercise_variant_version", "clinical_use_version", "protocol_version")] + [str(version_id)]
    plans = conn.execute(
        ...
        ([uuid.UUID(x) for x in ids if _is_uuid(x)], ids, ids)).fetchall()
    for p in plans:
        conn.execute("update plan_version set status='review_required', review_required_reason=%s where id=%s", (f"{entity_table} {version_id} withdrawn: {reason}", p["id"]))
        conn.execute("insert into outbox_event(event_type, tenant_id, object_table, object_id, object_version, payload) values ('plan.review_required',%s,'plan_version',%s,%s,%s)",
                     (p["tenant_id"], p["id"], p["revision"], J({"plan_id": str(p["plan_id"]), "reason": "dependency withdrawn"})))
    return {"affected_versions": sorted(f"{t}:{v}" for t, v in affected_versions), "plans_routed_to_review": [str(p["plan_id"]) for p in plans]}
