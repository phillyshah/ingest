...
def ensure_bmi_definition(conn: psycopg.Connection) -> dict:
    ...
    row = conn.execute("select * from segmentation_definition_version where dimension='bmi' and authority=%s and authority_version=%s",
                       (d["authority"], d["authority_version"])).fetchone()
    if row:
        return row
    return conn.execute(
        ...
        (d["authority"], d["authority_version"], d["population_scope"], J(d["boundaries"]), d["source_url"])).fetchone()
...
def derive_observations(conn: psycopg.Connection, tenant_id: Any, snapshot_id: Any, intake: Intake) -> list[dict[str, Any]]:
    ...
    def add(dimension: str, f, *, value=None, unit=None, status=None, classified=None, classification_version_id=None, method=None):
        o = {"dimension": dimension, "value": value if value is not None else (f.value if f else None), "unit": unit or (f.unit if f else None),
             "status": status or (f.status if f else FieldStatus.unknown), "provenance": (f.provenance if f else "derived"),
             "method": method or (f.provenance if f else "derived"), "classified_value": classified,
             "classification_version_id": classification_version_id, "reported_text": f.reported_text if f else None}
        obs.append(o)
        conn.execute(
            ...
            (tenant_id, snapshot_id, dimension, J(o["value"]), o["unit"], o["status"], f.measured_at if f else None, o["method"], o["provenance"],
             classification_version_id, classified))

    for dim in ("age", "sex", "gender", "height", "weight", "comorbidities", "functional_limitations", "equipment", "language"):
    ...
    if bmi_val is not None:
        ...
        add("bmi", None, value=round(bmi_val, 4), unit="kg/m2", status=FieldStatus.known, classified=cls.klass or f"not_classified: {cls.reason}",
            classification_version_id=bmi_def["id"], method="derived")
    else:
        add("bmi", bmi_f, status=bmi_f.status if bmi_f.status != FieldStatus.known else FieldStatus.unknown,
            classified=None, method="not_measured")
    return obs
...
def _pred_matches(pred: dict[str, Any], observations: dict[str, dict[str, Any]]) -> bool | None:
    ...
    try:
        return {"eq": v == val, "ne": v != val, "lt": v < val, "lte": v <= val, "gt": v > val, "gte": v >= val,
                "in": v in (val or []), "not_in": v not in (val or [])}[op]
    except TypeError:
        return None


def explain_segments(observations: list[dict[str, Any]], applicability: list[dict[str, Any]], bmi_def_version: str) -> list[SegmentExplanation]:
    ...
    for dim in ("age", "sex", "gender", "bmi", "comorbidities", "functional_limitations"):
        ...
        if o["status"] == FieldStatus.known and o["value"] is not None:
            ...
            observed = f"{o['status']}"
        exp = SegmentExplanation(dimension=dim, observed=observed, classification=o.get("classified_value"),
        ...
        for a in relevant:
            ...
            elif matched and a["action_type"] in ("adapt", "monitor", "exclude") and a["approval_state"] in ("approved", "published") and a["executable_rule_version_id"]:
