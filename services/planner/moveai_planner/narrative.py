...
CRITICAL_FIELDS = {"affected_side", "diagnosis_confirmation", "procedure", "procedure_type", "surgeon_restrictions", "severity",
                   "irritability", "concerning_findings", "prior_session_response", "associated_procedures", "brace_restrictions",
                   "weight_bearing_restrictions", "rom_restrictions"}

SEX_WORDS = {"man": "male", "male": "male", "gentleman": "male", "woman": "female", "female": "female", "lady": "female"}
BODY_SIZE_WORDS = ("slightly obese", "obese", "overweight", "heavy", "slim", "underweight", "thin", "athletic build")
...
def extract(narrative: str | None) -> tuple[dict[str, IntakeField], list[str]]:
    ...
    if m:
        fields["age"] = IntakeField(status=FieldStatus.known, value=int(m.group(1)), unit="years", provenance="narrative_extracted", reported_text=m.group(0))
    ...
    if side:
        fields["affected_side"] = IntakeField(status=FieldStatus.unknown, value=None, provenance="narrative_extracted", reported_text=side.group(1))
    ...
    if re.search(r"\b(surgery|surgical|repair|reconstruction|operat)", low):
        fields["procedure"] = IntakeField(status=FieldStatus.unknown, value=None, provenance="narrative_extracted", reported_text="surgical wording present")
    ...
    if re.search(r"\b(no surgery|nonoperative|non-operative|without surgery|conservative)", low):
        fields["procedure"] = IntakeField(status=FieldStatus.unknown, value=None, provenance="narrative_extracted", reported_text="nonoperative wording present")
...
def candidate_conditions(conditions: list[dict[str, Any]], narrative: str | None, intake: Intake) -> list[dict[str, Any]]:
    text = " ".join(x for x in [narrative or "", str(intake.value("diagnosis") or ""), str(intake.value("candidate_condition") or "")] if x).lower()
    if not text:
        return []
    hits = []
    for c in conditions:
