...
PERMANENT_ERRORS = {"rights_denied", "rights_unknown", "not_allowlisted", "schema_invalid", "quarantined", "ssrf_blocked"}
LEASE_SECONDS = 120
...
def enqueue(conn: psycopg.Connection, *, stage: str, source_version_id: Any = None, payload: dict | None = None,
            tenant_id: Any = None, campaign_id: Any = None, campaign_run_id: Any = None, priority: int = 100,
            extra_key: str = "") -> dict:
    ...
    row = conn.execute(
        ...
        (tenant_id, source_version_id, stage, key, J(payload or {}), campaign_id, campaign_run_id, priority)).fetchone()
    if row["inserted"]:
        conn.execute("insert into job_event(job_id, event, detail) values (%s,'enqueued',%s)", (row["id"], J({"stage": stage})))
    return row
...
def claim(conn: psycopg.Connection, worker_id: str, stages: tuple[str, ...] = STAGES) -> dict | None:
    row = conn.execute(
        """with c as (
           ...
           returning j.*""", (list(stages), worker_id, LEASE_SECONDS)).fetchone()
    if row:
        conn.execute("insert into job_event(job_id, event, detail) values (%s,'claimed',%s)", (row["id"], J({"worker": worker_id, "attempt": row["attempts"]})))
    return row


def heartbeat(conn: psycopg.Connection, job_id: Any) -> None:
    conn.execute("update ingestion_job set heartbeat_at = now(), leased_until = now() + make_interval(secs => %s) where id=%s",
...
def succeed(conn: psycopg.Connection, job_id: Any, *, warnings: list | None = None, cost_usd: float = 0, duration_ms: int = 0) -> None:
    conn.execute("""update ingestion_job set state='succeeded', finished_at=now(), warnings=%s, cost_usd = cost_usd + %s,
                    duration_ms=%s, leased_until=null where id=%s""", (J(warnings or []), cost_usd, duration_ms, job_id))
...
def fail(conn: psycopg.Connection, job_id: Any, error_class: str, message: str, *, permanent: bool | None = None) -> str:
    ...
    if permanent or job["attempts"] >= job["max_attempts"]:
        ...
        conn.execute("update ingestion_job set state=%s, error_class=%s, last_error=%s, finished_at=now(), leased_until=null where id=%s",
                     (state, error_class, message[:2000], job_id))
    else:
        delay = min(300, (2 ** job["attempts"]) + random.uniform(0, 1.0))
        state = "queued"
        conn.execute("update ingestion_job set state='queued', error_class=%s, last_error=%s, run_after = now() + make_interval(secs => %s), leased_until=null where id=%s",
                     (error_class, message[:2000], delay, job_id))
    conn.execute("insert into job_event(job_id, event, detail) values (%s,'failed',%s)",
...
def reclaim_expired(conn: psycopg.Connection) -> int:
    ...
    rows = conn.execute(
        """update ingestion_job set state='queued', leased_until=null, worker_id=null
            where state='running' and leased_until < now() returning id""").fetchall()
    for r in rows:
        conn.execute("insert into job_event(job_id, event, detail) values (%s,'reclaimed',%s)", (r["id"], J({"reason": "lease expired"})))
    return len(rows)


def retry_dead_letter(conn: psycopg.Connection, job_id: Any) -> bool:
    row = conn.execute("update ingestion_job set state='queued', attempts=0, run_after=now(), error_class=null where id=%s and state in ('dead_letter','failed') returning id",
                       (job_id,)).fetchone()
    if row:
