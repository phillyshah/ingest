...
def _pack_installed(conn: psycopg.Connection, pack: ContentPack) -> dict | None:
    return conn.execute(
        "select id from protocol_version where content_pack=%s and content_pack_version=%s limit 1",
        (pack.pack, pack.version),
    ).fetchone()
...
def install_pack(
    ...
    *,
    ...
    cond = conn.execute(
        ...
        (
            pack.condition.internal_code,
            pack.condition.preferred_name,
            pack.condition.synonyms,
            pack.condition.body_region,
            pack.condition.ambiguity_flags,
        ),
    ).fetchone()
    cond_id = cond["id"]
        ...
        for tc in conn.execute("select id from terminology_code where code=%s", (m["code"],)).fetchall():
            conn.execute(
                """insert into diagnosis_mapping_version(condition_id, terminology_code_id, relationship, confidence, approval_state)
                   values (%s,%s,%s,'high',%s) on conflict do nothing""",
                (cond_id, tc["id"], m.get("relationship", "exact"), state),
            )
    ...
    for s in pack.sources:
        src = conn.execute(
            ...
            (s.url, s.publisher, s.title, s.source_type, s.notes, ["reference"]),
        ).fetchone()
        sv = conn.execute(
            """insert into source_version(source_id, document_identity, declared_publication_date, pipeline_state)
               values (%s,%s,null,'access_checked') returning id""",
            (src["id"], f"{s.title} {s.document_version or ''}".strip()),
        ).fetchone()
        ...
        conn.execute(
            ...
            (sv["id"], *cols.values(), J(s.rights_evidence)),
        )
    ...
    for c in pack.claims:
        ...
        row = conn.execute(
            ...
            (
                ...
                c.paraphrase,
                c.claim_type,
                J(c.population) if c.population else None,
                c.recommendation_strength_as_reported,
                c.limitations,
            ),
        ).fetchone()
    ...
    for concept in pack.concepts:
        crow = conn.execute(
            ...
            (concept.preferred_name, concept.aliases, concept.body_region, concept.movement_purpose),
        ).fetchone()
        for v in concept.variants:
            ...
            src_ref: dict[str, Any] | None = {
                "url": None,
                "publisher": f"content pack {pack.pack} v{pack.version}",
                "title": pack.label,
                "document_version": pack.version,
                "locator": {"pack_variant_key": v.key},
                "source_exercise_name": v.name,
                "kind": "clinician_authored" if approved else "unsigned_placeholder",
            }
            if v.source_reference:
                ...
                src_ref = {
                    ...
                    "locator": v.source_reference.get("locator"),
                    "source_exercise_name": v.source_reference.get("source_exercise_name"),
                }
            vrow = conn.execute(
                ...
                (
                    ...
                    v.balance_demand,
                    v.setting,
                    J([{"step": i + 1, "text": s} for i, s in enumerate(v.step_sequence)]),
                    v.cues,
                    v.common_errors,
                    J(
                        {
                            "tags": v.tags,
                            "requires_starting_position": v.requires_starting_position,
                            "pack_key": v.key,
                        }
                    ),
                    J(src_ref) if src_ref else None,
                    vstate,
                    approver_id if approved else None,
            ...
            if v.media_state != "not_requested":
                conn.execute(
                    """insert into media_asset_version(entity_id, version, variant_version_id, media_type, media_state, approval_state)
                       values (%s,1,%s,'still_graphic',%s,%s)""",
                    (uuid.uuid4(), vrow["id"], v.media_state, "draft"),
                )
            ids["variants"][v.key] = {
                "id": vrow["id"],
                "entity_id": vrow["entity_id"],
                "tags": v.tags,
                "meta": v,
            }
    ...
    for u in pack.clinical_uses:
        ...
        for f in dose.values():  # resolve claim keys to ids
        ...
        urow = conn.execute(
            ...
            (
                uuid.uuid4(),
                tenant_id,
                ids["variants"][u.variant]["id"],
                cond_id,
                J({**u.presentation, "session_minutes_estimate": u.session_minutes_estimate}),
                u.indication,
                u.exclusion,
                u.phase,
                u.goals,
        ...
        for cid in claim_ids:
            conn.execute(
                "insert into clinical_use_evidence(clinical_use_version_id, evidence_claim_id, directness) values (%s,%s,%s) on conflict do nothing",
                (urow["id"], cid, "direct" if u.directness == "direct" else "indirect"),
            )
            conn.execute(
                "insert into dependency_edge(upstream_table, upstream_id, downstream_table, downstream_id, dependency_type) values ('evidence_claim',%s,'clinical_use_version',%s,'supported_by') on conflict do nothing",
                (cid, urow["id"]),
            )
        conn.execute(
            "insert into dependency_edge(upstream_table, upstream_id, downstream_table, downstream_id, dependency_type) values ('exercise_variant_version',%s,'clinical_use_version',%s,'uses_variant') on conflict do nothing",
    ...
    for r in pack.rules:
        ...
        rrow = conn.execute(
            ...
            (
                ...
                r.name,
                r.kind,
                J(r.expression.model_dump(by_alias=True, exclude_none=True)),
                r.required_inputs,
                J({**r.action.model_dump(exclude_none=True), "key": r.key}),
                r.severity,
                r.rationale,
                [ids["claims"][k] for k in r.evidence_claim_keys],
                author_id,
                approver_id if approved else None,
                rstate,
                "now()" if approved else None,
    ...
    for p in pack.protocols:
        ...
        for ph in p.phases:
            phases.append(
                {
                    **ph.model_dump(),
                    "items": [str(ids["uses"][k]) for k in ph.items],
        ...
        prow = conn.execute(
            ...
            (
                ...
                J(
                    {
                        **p.provenance,
                        "branch_conditions": p.branch_conditions,
                        "reassessment": p.reassessment,
                ...
                J(p.monitoring),
                rule_ids,
                J(p.recovery_horizon) if p.recovery_horizon else None,
                pstate,
                approver_id if approved else None,
                "now()" if approved else None,
            ),
        ...
        for ph in p.phases:
            for i, k in enumerate(ph.items):
                ...
                conn.execute(
                    "insert into dependency_edge(upstream_table, upstream_id, downstream_table, downstream_id, dependency_type) values ('clinical_use_version',%s,'protocol_version',%s,'uses_clinical_use') on conflict do nothing",
                    (ids["uses"][k], prow["id"]),
                )
        for rid in rule_ids:
            conn.execute(
                "insert into dependency_edge(upstream_table, upstream_id, downstream_table, downstream_id, dependency_type) values ('rule_version',%s,'protocol_version',%s,'uses_rule') on conflict do nothing",
                (rid, prow["id"]),
            )
    ...
    for a in pack.applicability:
        ...
        arow = conn.execute(
            ...
            (
                ...
                a.predicate_logic,
                a.relationship_type,
                [ids["claims"][k] for k in a.evidence_claims],
                a.support_category,
                a.generalizability_limits,
                a.action_type,
                ids["rules"][a.executable_rule] if a.executable_rule else None,
                a.clinician_rationale,
                astate,
            ),
        ).fetchone()
        for pred in a.predicates:
            conn.execute(
                ...
                (
                    ...
                    pred.get("unit"),
                    pred.get("min_inclusive"),
                    pred.get("max_inclusive"),
                ),
            )
    return summary(conn, pack)
...
def summary(conn: psycopg.Connection, pack: ContentPack) -> dict[str, Any]:
    rows = conn.execute(
        "select id, approval_state from protocol_version where content_pack=%s and content_pack_version=%s",
        (pack.pack, pack.version),
    ).fetchall()
    return {
        "pack": pack.pack,
        "version": pack.version,
        "approval_state": pack.approval_state.value,
