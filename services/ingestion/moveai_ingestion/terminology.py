...
def parse_order_file(path: Path) -> list[dict[str, Any]]:
    ...
    for line in path.read_text().splitlines():
        ...
        rows.append({"code": code, "descriptor": long_desc, "billable": flag == "1", "laterality": _laterality(long_desc)})
    return rows
    ...
    for rel in meta:
        ...
        row = conn.execute(
            """insert into terminology_release(system, release_id, effective_start, effective_end, source_hash, source_url)
               ...
               returning id""", (rel["system"], rel["release_id"], rel["effective_start"], rel["effective_end"], h, rel["source_url"])).fetchone()
...
def release_for_date(conn: psycopg.Connection, system: str, service_date: date) -> dict | None:
    return conn.execute(
        """select * from terminology_release where system=%s and effective_start <= %s and (effective_end is null or effective_end >= %s)
           order by effective_start desc limit 1""", (system, service_date, service_date)).fetchone()
...
def resolve_code(conn: psycopg.Connection, code: str, service_date: date, system: str = "ICD-10-CM") -> dict[str, Any]:
    ...
    if not rel:
        return {"code": code, "resolved": False, "note": f"no {system} release effective on {service_date}", "descriptor": None,
                "release_id": None, "release_label": None, "laterality": None, "billable": None}
    row = conn.execute("select * from terminology_code where release_id=%s and code=%s", (rel["id"], code.upper())).fetchone()
    if not row:
        return {"code": code, "resolved": False, "note": f"code not found in {rel['release_id']}", "descriptor": None,
                "release_id": str(rel["id"]), "release_label": rel["release_id"], "laterality": None, "billable": None}
    return {"code": row["code"], "resolved": True, "descriptor": row["descriptor"], "release_id": str(rel["id"]),
            "release_label": rel["release_id"], "laterality": row["laterality"], "billable": row["billable"],
