...
NEXT = {"access_check": "fetch", "fetch": "parse", "parse": "extract", "extract": "normalize", "normalize": "validate",
        "validate": "enqueue_review", "enqueue_review": None}
...
def stage_access_check(conn: psycopg.Connection, job: dict) -> dict:
    ...
    if src["allowlist_state"] != "approved":
        ...
        raise StageError("not_allowlisted", f"source {src['canonical_url']} allowlist_state={src['allowlist_state']}", permanent=True)
    ...
    return {"store_fulltext": check(grant, "can_store_fulltext").allowed, "store_excerpt": check(grant, "can_store_excerpt").allowed}
...
def stage_fetch(conn: psycopg.Connection, job: dict) -> dict:
    ...
    prior = conn.execute("select id from source_version where source_id=%s and content_sha256=%s and id<>%s and pipeline_state <> 'superseded' order by created_at desc limit 1",
                         (sv["source_id"], f.sha256, sv["id"])).fetchone()
    if prior:
        # unchanged bytes: link and skip re-extraction (spec §5.3)
        _set_state(conn, sv["id"], "superseded", content_sha256=f.sha256, retrieved_at="now()", final_url=f.final_url,
                   content_type=f.content_type, superseded_by_id=prior["id"], byte_size=len(f.content))
    ...
    _set_state(conn, sv["id"], "fetched", content_sha256=f.sha256, retrieved_at="now()", final_url=f.final_url, content_type=f.content_type,
               etag=f.etag, last_modified=f.last_modified, storage_ref=storage_ref, byte_size=len(f.content))
    return {"sha256": f.sha256, "content_type": f.content_type, "bytes": len(f.content), "stored": storage_ref is not None,
            "warnings": f.warnings, "url": url}
...
def stage_parse(conn: psycopg.Connection, job: dict) -> dict:
    ...
    _set_state(conn, sv["id"], "parsed", parser_version=PARSER_VERSION, parse_warnings=J(doc.warnings),
               declared_publication_date=doc.declared_publication_date, document_identity=doc.title or sv["document_identity"])
...
def stage_extract(conn: psycopg.Connection, job: dict, model: ExtractionModel | None = None) -> dict:
    ...
    return {"result": out.result.model_dump(mode="json"), "warnings": out.warnings, "review_flags": out.review_flags, "cost_usd": out.cost_usd}


def stage_normalize(conn: psycopg.Connection, job: dict) -> dict:
    sv, src = _source(conn, job["source_version_id"])
    ext = conn.execute("select checkpoint from ingestion_job where source_version_id=%s and stage='extract' and state='succeeded' order by finished_at desc limit 1",
                       (sv["id"],)).fetchone()
    if not ext:
    ...
    if conn.execute("select 1 from exercise_variant_version v join dependency_edge d on d.downstream_id=v.id where d.upstream_id=%s limit 1", (sv["id"],)).fetchone():
        return {"already_normalized": True}
    created = persist(conn, source_version_id=sv["id"], source_row=src, result=result, review_flags=flags, region=region,
                      excerpt_allowed=check(grant, "can_store_excerpt").allowed, tenant_id=job["tenant_id"])
...
def stage_validate(conn: psycopg.Connection, job: dict) -> dict:
    sv, src = _source(conn, job["source_version_id"])
    issues: list[str] = []
    rows = conn.execute("select v.* from exercise_variant_version v join dependency_edge d on d.downstream_id=v.id and d.upstream_table='source_version' where d.upstream_id=%s", (sv["id"],)).fetchall()
    for v in rows:
    ...
    conflicts = conn.execute("select count(*) as n from evidence_claim where source_version_id=%s and conflicts_with_claim_id is not null", (sv["id"],)).fetchone()["n"]
...
def stage_enqueue_review(conn: psycopg.Connection, job: dict) -> dict:
    ...
    n = conn.execute("select count(*) as n from exercise_variant_version v join dependency_edge d on d.downstream_id=v.id and d.upstream_table='source_version' where d.upstream_id=%s and v.approval_state='pending_review'",
                     (sv["id"],)).fetchone()["n"]
    return {"queued_for_review": n}


STAGE_FN = {"access_check": stage_access_check, "fetch": stage_fetch, "parse": stage_parse, "extract": stage_extract,
            "normalize": stage_normalize, "validate": stage_validate, "enqueue_review": stage_enqueue_review}
    ...
    try:
        try:
            with conn.transaction():
                ...
                q.succeed(conn, job["id"], warnings=result.get("warnings", []) + result.get("review_flags", []),
                          cost_usd=float(result.get("cost_usd", 0)), duration_ms=int((time.monotonic() - t0) * 1000))
                nxt = NEXT[job["stage"]]
                if nxt and not result.get("skip_next"):
                    q.enqueue(conn, stage=nxt, source_version_id=job["source_version_id"], payload=job["payload"], tenant_id=job["tenant_id"],
                              campaign_id=job["campaign_id"], campaign_run_id=job["campaign_run_id"], priority=job["priority"])
            state = "succeeded"
...
def run_all(conn: psycopg.Connection, *, worker_id: str = "inline", model: ExtractionModel | None = None, max_jobs: int = 500,
            commit: bool = False) -> list[tuple[str, str]]:
...
def register_source_version(conn: psycopg.Connection, source_id: Any, *, url: str | None = None) -> Any:
    return conn.execute("insert into source_version(source_id, final_url, pipeline_state) values (%s,%s,'discovered') returning id",
                        (source_id, url)).fetchone()["id"]
