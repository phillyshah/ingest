...
ALL_ALLOWED = {k: "allowed" for k in ("can_fetch", "can_store_fulltext", "can_store_excerpt", "can_embed_for_search", "can_process_with_model",
                                      "can_display_to_clinician", "can_display_to_patient", "can_redistribute", "can_transform")}


def _register_and_ingest(admin, conn, name="owned_demo_protocol.html", rights=ALL_ALLOWED, approve=True):
    r = admin.post("/sources", json={"canonical_url": "https://example.invalid/x", "source_type": "html", "publisher": "MoveAI", "title": name,
                                     "local_path": f"{FIX / name}?t={uuid.uuid4().hex[:6]}", "rights": {"permissions": rights, "permission_evidence": {"kind": "ownership"}}})
...
def test_source_to_review_to_publish(admin, pt, pt2, lead, conn):
    ...
    assert d["duplicate_proposal"] is not None
    r = pt.post("/reviews", json={"entity_table": "exercise_variant_version", "version_id": vid, "decision": "approve"})
    assert r.status_code == 409 and r.json()["code"] == "duplicate_unresolved"
    r = pt.post("/reviews", json={"entity_table": "exercise_variant_version", "version_id": vid, "decision": "reject", "reason": "duplicate of catalog variant"})
    ...
    vid = next(i["version_id"] for i in pt.get("/reviews/queue?entity_table=exercise_variant_version").json()["items"] if i["name"].startswith("Movement A"))
    r = pt.post("/reviews", json={"entity_table": "exercise_variant_version", "version_id": vid, "decision": "approve", "reason": "ok", "time_spent_seconds": 40})
    assert r.status_code == 201 and r.json()["approval_state"] == "approved"
    r = pt2.post("/reviews", json={"entity_table": "exercise_variant_version", "version_id": vid, "decision": "edit", "changes": {"cues": ["keep it slow"]}, "reason": "clarity"})
    assert r.status_code == 201 and r.json()["invalidated_prior_approval"] and r.json()["approval_state"] == "pending_review"
    assert conn.execute("select approval_state from exercise_variant_version where id=%s", (vid,)).fetchone()["approval_state"] == "invalidated"
    new_vid = r.json()["new_version_id"]
    # author of the new version cannot approve it
    assert pt2.post("/reviews", json={"entity_table": "exercise_variant_version", "version_id": new_vid, "decision": "approve"}).status_code == 403
    assert pt.post("/reviews", json={"entity_table": "exercise_variant_version", "version_id": new_vid, "decision": "approve"}).status_code == 201
...
def test_rights_unknown_source_is_held_and_visible(admin, conn):
    ...
    job = admin.get(f"/ingestion-jobs/{job_id}").json()
    assert job["state"] == "failed" and job["error_class"] == "rights_unknown" and job["pipeline_state"] == "rights_hold" and not job["retry_eligible"]


def test_merge_refuses_distinct_assistance(admin, pt, conn):
    _register_and_ingest(admin, conn)
    rows = conn.execute("select id, entity_id, assistance from exercise_variant_version where name like 'Demo movement%%' and approval_state='pending_review' order by name").fetchall()
    a = next(r for r in rows if r["assistance"] == "active"); b = next(r for r in rows if r["assistance"] == "resisted")
    r = pt.post("/reviews", json={"entity_table": "exercise_variant_version", "version_id": str(b["id"]), "decision": "merge", "merge_into_entity_id": str(a["entity_id"])})
...
def test_rights_denial_holds_media_and_withdrawal_propagates(admin, pt, rights, lead, conn):
    ...
    media = conn.execute("select m.id, m.variant_version_id from media_asset_version m join exercise_variant_version v on v.id=m.variant_version_id "
                         "join dependency_edge d on d.downstream_id=v.id where d.upstream_id=%s", (svid,)).fetchone()
    assert conn.execute("select media_state from media_asset_version where id=%s", (media["id"],)).fetchone()["media_state"] == "rights_hold"
    # text approval proceeds regardless of the graphic
    assert pt.post("/reviews", json={"entity_table": "exercise_variant_version", "version_id": str(media["variant_version_id"]), "decision": "approve"}).status_code == 201
    # withdrawal of the source propagates
    assert pt.post("/reviews", json={"entity_table": "source_version", "version_id": svid, "decision": "withdraw", "reason": "publisher retraction"}).status_code == 403
    r = lead.post("/reviews", json={"entity_table": "source_version", "version_id": svid, "decision": "withdraw", "reason": "publisher retraction"})
    assert r.status_code == 201 and any(v.startswith("exercise_variant_version") for v in r.json()["propagation"]["affected_versions"])
    assert conn.execute("select approval_state from exercise_variant_version where id=%s", (media["variant_version_id"],)).fetchone()["approval_state"] == "withdrawn"


def test_plan_options_roles_and_three_families(pt, admin, integration):
    for text in ("55-year-old slightly obese man with frozen shoulder", "Patient recovering after surgical MCL repair", "Patient with a hamstring pull, no surgery"):
        r = pt.post("/plan-options", json={"case_ref": f"c-{uuid.uuid4().hex[:6]}", "narrative": text})
        assert r.status_code == 200, r.text
        b = r.json()
        assert b["status"] == "needs_assessment" and b["prescription"] is None and b["assignable"] is False and b["pathway_previews"] and b["missing_fields"]
...
def test_plan_draft_approve_retrieve_via_api(pt, admin, integration, seeded):
    fields = {"affected_side": {"status": "known", "value": "left"}, "concerning_findings": {"status": "known", "value": []},
              "surgeon_restrictions": {"status": "known", "value": "none"}, "equipment": {"status": "known", "value": ["elastic band"]},
              "prior_session_response": {"status": "not_applicable"}, "age": {"status": "known", "value": 55, "unit": "years"}}
    r = pt.post("/plan-options", json={"case_ref": "api-demo", "narrative": "demo condition", "intake": {"fields": fields}})
    assert r.status_code == 200 and r.json()["status"] == "draft_ready", r.text
    plan_id = r.json()["plan_id"]; opt = r.json()["options"][0]
    ...
    a = pt.post(f"/plans/{plan_id}/approve", json={"expected_revision": 2, "attestation": "reviewed"}, headers={"Idempotency-Key": "k1"})
    assert a.status_code == 200 and a.json()["status"] == "approved"
    replay = pt.post(f"/plans/{plan_id}/approve", json={"expected_revision": 2, "attestation": "reviewed"}, headers={"Idempotency-Key": "k1"})
    assert replay.status_code == 200 and replay.headers.get("idempotent-replay") == "true"
    assert pt.post(f"/plans/{plan_id}/approve", json={"expected_revision": 2, "attestation": "other"}, headers={"Idempotency-Key": "k1"}).status_code == 409
    ...
    assert other.get(f"/plans/{plan_id}/approved").status_code in (403, 404) or other.get(f"/plans/{plan_id}/approved").json()["available"] is False
...
def As_other(base, seeded):
    class O:
        def get(self, url):
            return base.c.get("/v1" + url, headers={"X-User-Id": str(seeded["integration"]), "X-Tenant-Id": str(uuid.uuid4())})
    return O()


def test_protocol_draft_rejects_unsupported_rules(pt, seeded):
    body = {"condition_code": "adhesive_capsulitis", "name": "Tenant pathway", "rules": [{"key": "r1", "name": "bad", "expression": {"op": "regex", "field": "age", "value": ".*"}, "action": {"type": "inform"}}]}
    assert pt.post("/protocols", json=body).status_code == 422
    body["rules"] = [{"key": "r1", "name": "ok", "expression": {"op": "known", "field": "affected_side"}, "action": {"type": "require_field", "field": "affected_side", "reason": "side"}}]
...
def test_campaign_lifecycle(admin, pt, lead, conn):
    scope = {"ailment_text": "surgically repaired MCL tear", "codes": ["S83.411A"], "limits": {"max_sources": 2, "max_usd": 1.0, "max_search_requests": 10},
             "supplied_source_urls": [f"file://{FIX / 'owned_demo_protocol.html'}?c={uuid.uuid4().hex[:6]}", "https://unknown.example/protocol.pdf"],
             "source_policy": "allowlist_and_supplied", "service_date": "2026-09-09"}
    ...
    assert pv["resolved_codes"][0]["resolved"] and pv["resolved_codes"][0]["release_label"] == "FY26" and "right" == pv["resolved_codes"][0]["laterality"]
    ...
    dispatched = conn.execute("select s.allowlist_state from ingestion_job j join source_version sv on sv.id=j.source_version_id join source s on s.id=sv.source_id where j.campaign_id=%s", (cid,)).fetchall()
    ...
    assert c["lifecycle"] == "needs_attention" and any("pending allowlist" in b for b in c["blockers"]) and c["runs"][-1]["state"] == "finished"
    ...
    assert [e["event"] for e in ev if e["event"] != "job"] == ["created", "scope_revised", "started", "paused", "resumed", "started", "cancelled"]
...
def test_campaign_with_approved_source_runs_and_limits_hold(admin, pt, conn):
    ...
    r = admin.post("/sources", json={"canonical_url": url, "source_type": "html", "publisher": "MoveAI", "title": "owned", "rights": {"permissions": ALL_ALLOWED}})
    sid = r.json()["id"]; admin.patch(f"/sources/{sid}", json={"allowlist_state": "approved"})
    scope = {"ailment_text": "demo condition", "limits": {"max_sources": 1, "max_usd": 0.04, "max_search_requests": 10}, "supplied_source_urls": [url, url + "&dup=1"],
             "source_policy": "supplied_only", "scope_confirmed": True}
    ...
    c = admin.patch(f"/ingestion-campaigns/{c['id']}", json={"scope": {**scope, "limits": {"max_sources": 1, "max_usd": 1.0, "max_search_requests": 10}}}).json()
    c = admin.post(f"/ingestion-campaigns/{c['id']}/runs").json()
