...
app = FastAPI(title="MoveAI Exercise Ingestion and Plan-Drafting Engine", version="0.1.0", docs_url="/v1/docs", openapi_url="/v1/openapi.json",
...
def diagnostics(p: Principal = Depends(current_principal), conn: psycopg.Connection = Depends(get_conn)) -> dict[str, Any]:
    ...
    ob = conn.execute("select count(*) filter (where delivered_at is null) as undelivered, count(*) filter (where dead_lettered) as dead from outbox_event").fetchone()
    return {"queue": q, "outbox": ob, "release": (conn.execute("select label, published_at from catalog_release order by published_at desc limit 1").fetchone()),
...
def users(p: Principal = Depends(current_principal), conn: psycopg.Connection = Depends(get_conn)) -> dict[str, Any]:
    ...
    rows = conn.execute("select id, email, display_name, roles::text[] as roles, tenant_id from app_user where tenant_id=%s or %s is null order by display_name", (p.tenant_id, p.tenant_id)).fetchall()
    return {"items": [{"id": str(r["id"]), "email": r["email"], "display_name": r["display_name"], "roles": r["roles"], "tenant_id": str(r["tenant_id"]) if r["tenant_id"] else None} for r in rows]}
