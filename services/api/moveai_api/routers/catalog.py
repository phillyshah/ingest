...
def search_exercises(q: str | None = None, region: str | None = None, condition: str | None = None, assistance: str | None = None, phase: str | None = None,
                     equipment: str | None = None, setting: str | None = None, language: str | None = None, patient_display: bool | None = None,
                     review_status: str | None = None, include_unpublished: bool = False, limit: int = Query(50, le=200),
    ...
    if include_unpublished and p.has("pt", "clinical_lead", "source_admin", "rights_reviewer", "auditor"):
        states = ["draft", "pending_review", "approved", "published", "unsigned_placeholder", "invalidated", "rejected", "withdrawn", "superseded"]
    if review_status:
    ...
    rows = conn.execute(
        ...
        (states, q, q, q, q, region, region, assistance, assistance, equipment, equipment, setting, setting, language, language,
         condition, condition, condition, phase, phase, limit + 1)).fetchall()
    out = []
    for r in rows:
        media = r["media"] or []
        item = clean(r)
        item["media_quality"] = {"graphic_available": any(m["media_state"] == "graphic_available" for m in media), "states": [m["media_state"] for m in media]}
        item["clinical_confidence"] = {"uses": len(r["uses"] or []), "directness": sorted({u["directness"] for u in (r["uses"] or [])}),
...
def get_version(entity_id: str, version: int, conn: psycopg.Connection = Depends(get_conn), p: Principal = Depends(current_principal)) -> dict[str, Any]:
    v = conn.execute("select * from exercise_variant_version where entity_id=%s and version=%s", (as_uuid(entity_id), version)).fetchone()
    if not v:
...
def get_entity(entity_id: str, conn: psycopg.Connection = Depends(get_conn), p: Principal = Depends(current_principal)) -> dict[str, Any]:
    vs = conn.execute("select * from exercise_variant_version where entity_id=%s order by version desc", (as_uuid(entity_id),)).fetchall()
    if not vs:
        raise HTTPException(404, {"code": "not_found", "message": "exercise not found"})
    return {"entity_id": entity_id, "latest": detail(conn, "exercise_variant_version", vs[0]["id"]), "versions": clean_all(vs)}


@router.get("/reviews/queue")
def review_queue(entity_table: str | None = None, campaign_id: str | None = None, limit: int = Query(50, le=200), cursor: str | None = None,
                 conn: psycopg.Connection = Depends(get_conn), p: Principal = Depends(require("pt", "clinical_lead", "rights_reviewer", "auditor"))) -> dict[str, Any]:
...
def review_detail(entity_table: str, version_id: str, conn: psycopg.Connection = Depends(get_conn),
                  p: Principal = Depends(require("pt", "clinical_lead", "rights_reviewer", "auditor", "source_admin"))) -> dict[str, Any]:
    if entity_table not in ("exercise_variant_version", "clinical_use_version", "protocol_version", "rule_version", "media_asset_version", "rights_grant", "source_version"):
...
def create_review(body: ReviewCreate, p: Principal = Depends(current_principal), conn: psycopg.Connection = Depends(get_conn)) -> dict[str, Any]:
    return decide(conn, p, body)


@router.post("/protocols", status_code=201)
def create_protocol(body: ProtocolCreate, p: Principal = Depends(require("pt", "clinical_lead")), conn: psycopg.Connection = Depends(get_conn)) -> dict[str, Any]:
    ...
    for r in body.rules:
        ...
        rid = conn.execute(
            ...
            (_uuid.uuid4(), p.tenant_id if body.tenant_private else None, r.get("name", r.get("key", "rule")), r.get("kind", "eligibility"),
             J(expr.model_dump(by_alias=True, exclude_none=True)), sorted(expr.fields()), J({**action.model_dump(exclude_none=True), "key": r.get("key")}),
             r.get("severity", "info"), r.get("rationale"), p.user_id)).fetchone()["id"]
        rule_ids.append(rid)
        previews.append(f"IF {_readable(expr.model_dump(by_alias=True, exclude_none=True))} THEN {action.type} {action.message or action.field or action.fields or ''}".strip())
    for ph in body.phases:
        for uid in ph.get("items", []):
            ...
            if not u or u["approval_state"] not in ("approved", "published"):
                raise HTTPException(422, {"code": "unapproved_item", "message": f"clinical use {uid} is not approved; pathways use approved variants only"})
    row = conn.execute(
        ...
        (_uuid.uuid4(), p.tenant_id if body.tenant_private else None, cond["id"], body.name, body.population_description, p.user_id,
         J({**body.provenance, "kind": body.provenance.get("kind", "clinician_authored"), "pack_rules": [str(r) for r in rule_ids]}), body.setting,
         body.required_inputs, J(body.phases), J(body.monitoring), rule_ids, J(body.recovery_horizon) if body.recovery_horizon else None)).fetchone()
    for ph in body.phases:
        for i, uid in enumerate(ph.get("items", [])):
            conn.execute("insert into protocol_item(protocol_version_id, phase_key, clinical_use_version_id, position) values (%s,%s,%s,%s)", (row["id"], ph["key"], uid, i))
    return {"protocol_version_id": str(row["id"]), "rule_version_ids": [str(r) for r in rule_ids], "rule_previews": previews, "approval_state": "draft",
...
def list_protocols(condition: str | None = None, conn: psycopg.Connection = Depends(get_conn), p: Principal = Depends(current_principal)) -> dict[str, Any]:
    rows = conn.execute(
        ...
        (condition, condition, p.tenant_id)).fetchall()
    return {"items": clean_all(rows), "total": len(rows)}


@router.post("/catalog-releases", status_code=201)
def create_release(body: CatalogReleaseCreate, p: Principal = Depends(require("clinical_lead")), conn: psycopg.Connection = Depends(get_conn)) -> dict[str, Any]:
...
def list_releases(conn: psycopg.Connection = Depends(get_conn), p: Principal = Depends(current_principal)) -> dict[str, Any]:
    rows = conn.execute("select id, label, manifest_sha256, published_at, prior_release_id, jsonb_array_length(manifest) as item_count from catalog_release order by published_at desc").fetchall()
    return {"items": clean_all(rows), "total": len(rows)}


@router.get("/changes")
def changes(cursor: int = 0, limit: int = Query(100, le=1000), conn: psycopg.Connection = Depends(get_conn), p: Principal = Depends(current_principal)) -> dict[str, Any]:
...
def resolve(code: str, service_date: str | None = None, conn: psycopg.Connection = Depends(get_conn), p: Principal = Depends(current_principal)) -> dict[str, Any]:
