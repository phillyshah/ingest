...
def create_plan_options(
    body: CaseSubmission,
    p: Principal = Depends(require("pt", "clinical_lead", "integration")),
...
def get_plan(
    plan_id: str,
    p: Principal = Depends(require("pt", "clinical_lead", "auditor")),
    conn: psycopg.Connection = Depends(get_conn),
) -> dict[str, Any]:
...
def approved_plan(
    plan_id: str,
    p: Principal = Depends(require("integration", "pt", "clinical_lead", "auditor")),
    conn: psycopg.Connection = Depends(get_conn),
) -> dict[str, Any]:
