...
def _draft_and_approve(conn, s):
    ...
    fields = {
        "affected_side": IntakeField.known("left"),
        "concerning_findings": IntakeField.known([]),
        "surgeon_restrictions": IntakeField.known("none"),
        "equipment": IntakeField.known(["elastic band"]),
        "prior_session_response": IntakeField(status="not_applicable"),
    }
    r = plan_options(
        conn,
        tenant_id=s["tenant_id"],
        user_id=s["pt"],
        submission=CaseSubmission(case_ref="adapter", narrative="demo condition", intake=Intake(fields=fields)),
    )
    patch_draft(
        conn,
        tenant_id=s["tenant_id"],
        user_id=s["pt"],
        plan_id=r.plan_id,
        patch=DraftPatch(expected_revision=1, selected_option_id=r.options[0].option_id),
    )
    approve(
        conn,
        tenant_id=s["tenant_id"],
...
def test_adapter_reads_only_approved_and_dedups(conn):
    ...
    try:
        with TestClient(app, base_url="http://testserver/v1") as tc:
            client = MoveAIMockClient(
                base_url="http://testserver/v1",
                user_id=str(s["integration"]),
