...
def _concept(conn):
    return conn.execute("insert into exercise_concept(preferred_name, body_region) values (%s,'shoulder') returning id",
                        (f"concept-{uuid.uuid4().hex[:8]}",)).fetchone()["id"]


def _variant(conn, concept_id, state="draft", **over):
    row = dict(concept_id=concept_id, entity_id=uuid.uuid4(), version=1, name="Assisted ER", region="shoulder",
               assistance="active_assisted", approval_state=state)
    row.update(over)
...
def test_approved_variant_is_immutable(conn):
    ...
    conn.execute("update exercise_variant_version set approval_state='withdrawn', withdrawn_at=now(), withdrawal_reason='test' where id=%s", (vid,))
    assert conn.execute("select approval_state from exercise_variant_version where id=%s", (vid,)).fetchone()["approval_state"] == "withdrawn"
...
def test_dose_shape_constraint(conn):
    ...
    cond = conn.execute("insert into condition(internal_code, preferred_name, body_region) values (%s,'x','shoulder') returning id",
                        (f"c-{uuid.uuid4().hex[:6]}",)).fetchone()["id"]
    good = {"sets": {"value": 3, "provenance": "clinician_authored"},
            "repetitions": {"value": None, "null_reason": "not in source", "provenance": "unknown"}}
    conn.execute("insert into clinical_use_version(entity_id, version, variant_version_id, condition_id, dose_envelope) values (%s,1,%s,%s,%s)",
...
def test_author_cannot_approve_own_rule(conn, users):
    with pytest.raises(psycopg.errors.CheckViolation):
        with conn.transaction():
            conn.execute("insert into rule_version(entity_id, version, name, rule_kind, expression, action, author_id, approver_id) "
                         "values (%s,1,'r','eligibility','{}','{}',%s,%s)", (uuid.uuid4(), users["pt"], users["pt"]))


def test_applicability_action_requires_executable_rule(conn):
    with pytest.raises(psycopg.errors.CheckViolation):
        with conn.transaction():
            conn.execute("insert into population_applicability_version(target_version_id, target_type, relationship_type, action_type) "
                         "values (%s,'clinical_use','precaution','exclude')", (uuid.uuid4(),))


def test_budget_reservation_is_atomic_and_bounded(conn, tenants, users):
    camp = conn.execute("insert into ingestion_campaign(tenant_id, title) values (%s,'t') returning id", (tenants["tenant_a"],)).fetchone()["id"]
    scope = conn.execute(
        "insert into campaign_scope_version(campaign_id, version, desired_output, limits) values (%s,1,'x',%s) returning id",
        (camp, J({"max_usd": 1.0, "max_search_requests": 2, "max_document_bytes": 1000, "max_sources": 5,
                  "max_candidates": 5, "max_runtime_seconds": 60}))).fetchone()["id"]
    run = conn.execute("insert into campaign_run(campaign_id, scope_version_id, run_number) values (%s,%s,1) returning id",
                       (camp, scope)).fetchone()["id"]
