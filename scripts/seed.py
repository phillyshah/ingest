...
def ingest_fixture(conn, name: str, rights: dict[str, str], source_type: str = "html", tenant_id=None):
    ...
    if src:
        return None
    src = conn.execute("insert into source(canonical_url, publisher, title, source_type, allowlist_state, intended_uses) values (%s,'MoveAI (owned synthetic fixture)',%s,%s,'approved','{reference,clinician}') returning *",
                       (url, name, source_type)).fetchone()
    ...
    q.enqueue(conn, stage="access_check", source_version_id=svid, payload={"url": url, "region": "demo"}, tenant_id=tenant_id)
    return svid
...
if __name__ == "__main__":
    with connect() as conn:
        ...
        for name, rights, st in (("owned_demo_protocol.html", ALL, "html"), ("owned_demo_protocol.pdf", ALL, "pdf"), ("headingless_dose_table.html", ALL, "html"),
                                 ("injection_attempt.html", ALL, "html"), ("rights_restricted_graphic.html", TEXT_ONLY, "html"),
                                 ("scanned_ambiguous.ocr.json", ALL, "scanned_pdf")):
            ingest_fixture(conn, name, rights, st, out["tenant_id"])
        done = run_all(conn)
        conn.commit()
        print(f"ingestion: {sum(1 for _, s in done if s == 'succeeded')} stages succeeded, {sum(1 for _, s in done if s != 'succeeded')} held/failed")
