...
CANONICAL = ["55-year-old slightly obese man with frozen shoulder", "Patient recovering after surgical MCL repair",
...
def main() -> int:
    ...
    with connect() as conn:
        ...
        if not rel:
            print("run `make seed` first"); return 2
        ...
        print("  media binaries stored:", conn.execute("select count(*) as n from media_asset_version where storage_ref is not null").fetchone()["n"], "(must be 0)")
        ...
        for text in CANONICAL:
            r = plan_options(conn, tenant_id=ctx["tenant_id"], user_id=ctx["pt"], submission=CaseSubmission(case_ref=f"demo-{uuid.uuid4().hex[:6]}", narrative=text))
            (OUT / f"plan-options-{text[:20].replace(' ', '_')}.json").write_text(r.model_dump_json(indent=2))
            print(f"  '{text}'")
            print(f"    status={r.status.value} assignable={r.assignable} prescription={r.prescription} previews={len(r.pathway_previews)} anchor={r.timeline_anchor}")
            print(f"    missing: {[m.field for m in r.missing_fields]}")
            for p in r.pathway_previews[:1]:
                for row in p.timeline[:1]:
                    print(f"    preview '{p.protocol_name}' [{p.approval_state}] phase '{row.phase_name}' window={row.time_window.text!r} exercises={[e.variant_name for e in row.exercises]}")
                    for e in row.exercises[:1]:
                        print(f"      source ref: {e.source_references[0].publisher} / {e.source_references[0].title} locator={e.source_references[0].locator}")
        ...
        fs = {"affected_side": K("right"), "diagnosis_confirmation": K({"confirmed": True}), "irritability": K("low"), "rom_active": K({"flexion": 100}, unit="deg"),
              "rom_passive": K({"flexion": 110}, unit="deg"), "surgeon_restrictions": K("none"), "procedure": K("none"), "goals": K(["reach behind head"]),
              "prior_interventions": K([]), "concerning_findings": K([]), "age": K(55, unit="years"), "prior_session_response": IntakeField(status=FieldStatus.not_applicable)}
        r = plan_options(conn, tenant_id=ctx["tenant_id"], user_id=ctx["pt"], submission=CaseSubmission(case_ref="demo-fs-full", narrative="frozen shoulder", intake=Intake(fields=fs)))
        ...
        demo = {"affected_side": K("left"), "concerning_findings": K([]), "surgeon_restrictions": K("none"), "equipment": K(["elastic band"]),
                "height": K(1.75, unit="m"), "weight": K(96, unit="kg"), "age": K(55, unit="years"), "prior_session_response": IntakeField(status=FieldStatus.not_applicable)}
        r = plan_options(conn, tenant_id=ctx["tenant_id"], user_id=ctx["pt"], submission=CaseSubmission(case_ref="demo-approve", narrative="demo condition", intake=Intake(fields=demo)))
        ...
        opt = r.options[0]
        print(f"  option '{opt.label}': items={[i.variant_name for i in opt.items]} validation.passed={opt.validation.passed} media={[i.media_state for i in opt.items]}")
        rev2 = patch_draft(conn, tenant_id=ctx["tenant_id"], user_id=ctx["pt"], plan_id=r.plan_id, patch=DraftPatch(expected_revision=1, selected_option_id=opt.option_id, rationale=narrative_for(opt)))
        approved = approve(conn, tenant_id=ctx["tenant_id"], user_id=ctx["pt"], roles=["pt"], plan_id=r.plan_id, expected_revision=rev2["revision"], attestation="demo")
        got = get_approved(conn, tenant_id=ctx["tenant_id"], plan_id=r.plan_id)
        print(f"  approved revision={approved['revision']} signature={approved['approval_signature'][:16]}... adapter available={got['available']} hash_match={got['content_hash'] == approved['content_hash']}")
        ...
        print(f"  adapter now: available={got2['available']} hold={got2['hold']} assignable={got2.get('assignable')} reason={got2['hold_reason']}")
